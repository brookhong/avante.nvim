...
function M.init()
  ...
  -- read entries from avante_prompts.log and sort them by entry time
  ...
  if file then
    ...
    for _, line in ipairs(lines) do
      ...
      if entry and entry.time and entry.input then
...
function M.log_prompt(request)
  ...
  if #entries > 1 then
    for i = #entries - 1, 1, -1 do
      if entries[i].input == entry.input then
...
function M.on_log_retrieve(delta)
  return function()
    ...
    vim.api.nvim_win_set_cursor(0, {vim.api.nvim_buf_line_count(0), #vim.api.nvim_buf_get_lines(0, -2, -1, false)[1]})
  end
end
