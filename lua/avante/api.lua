...
function M.explain(opts)
  ...
  if spinner.spinner_active then
    return
  end
  ...
  if not content_to_explain then
    ...
    if selection then
      if selection.range.start.lnum == selection.range.finish.lnum and selection.range.start.col < selection.range.finish.col then
  ...
  local system_prompt = opts.system_prompt and opts.system_prompt or [[
...
Keep your explanation clear and to the point.
  ...
  local user_template = opts.user_template and opts.user_template or
    "The code is written in <FILE_TYPE>.\nExplain this code:\n\n<SELECTION>"
  local user_prompt = user_template:gsub("<FILE_TYPE>", filetype)
    :gsub("<SELECTION>", content_to_explain) or content_to_explain
  ...
  local function create_window_and_buffer()
    ...
    if screen_col + width > vim.o.columns then
      screen_col = vim.o.columns - width - 2
    end
    if screen_row + height > vim.o.lines then
      screen_row = vim.o.lines - height - 2
    end
  ...
  Llm.curl({
    ...
    handler_opts = {
      ...
      on_chunk = function(chunk)
        ...
        if not result_bufnr or not api.nvim_buf_is_valid(result_bufnr) then
          return
        end
