...
function CursorSpinner:update()
  ...
  if not buffer_is_visible then
    return
  end

  -- Remove previous extmark if it exists
  if self.extmark_id then
    pcall(api.nvim_buf_del_extmark, self.bufnr, namespace, self.extmark_id)
  end
  ...
  self.extmark_id = api.nvim_buf_set_extmark(self.bufnr, namespace, self.row, self.col, opts)
end
